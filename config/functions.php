...
function sendMultiNotification($user_id, $title, $message, $type = 'test', $channels = ['website', 'email']) {
    $database = new Database();
    $db = $database->getConnection();
    
    // Get user info
    $stmt = $db->prepare("SELECT name, email, telegram_chat_id FROM users WHERE id = ?");
    $stmt->execute([$user_id]);
    $user = $stmt->fetch();
...
function checkTasksAndSendReminders() {
    try {
        ...
        $stmt = $db->prepare("
            SELECT t.*, u.name as user_name, u.email, u.telegram_chat_id,
                   us.notification_email, us.notification_telegram
            FROM tasks t
